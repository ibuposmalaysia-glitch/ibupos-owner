...
function cors(req, res) {
  ...
  res.setHeader(
    ...
    "Content-Type,Authorization,X-Device-Key,X-Outlet-Id,X-Github-Token"
  );
...
function githubToken(req) {
  const t = String(req.headers["x-github-token"] || "").trim();
  if (t) {
    try {
      require("./store").rememberToken(t);
    } catch {
      /* ignore */
    }
  }
  return t || process.env.GITHUB_TOKEN || "";
}
